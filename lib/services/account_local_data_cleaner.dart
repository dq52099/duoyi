...
/// Clears local data that belongs to the signed-in account.
///
/// Device-level settings such as locale and app lock are intentionally kept.
/// Account data, sync metadata, notification history, backup targets and
/// cached integration payloads must not survive logout or account switching.
class AccountLocalDataCleaner {
  ...
  static Future<void> clearLocalFiles() async {
    ...
    try {
      ...
      await _clearDocumentDirectories(root);
    } catch (e, st) {
      debugPrint('[AccountLocalDataCleaner] clear local files failed: $e\n$st');
    }
  }

  @visibleForTesting
  static Future<void> clearDocumentDirectoriesForTest(Directory root) {
    return _clearDocumentDirectories(root);
  }

  static Future<void> _clearDocumentDirectories(Directory root) async {
    for (final name in accountScopedDocumentDirectories) {
      ...
      try {
        ...
        debugPrint(
          '[AccountLocalDataCleaner] delete ${dir.path} failed: $e\n$st',
        );
      }
    }
  }
