...
enum _ForegroundPopupShowResult { shown, unavailable, suppressed }
...
class ForegroundReminderPopupSink implements ReminderPopupSink {
  ...
  void _showOrFallback({
    ...
    final result = isForegroundGetter()
        ? _show(id: id, title: title, body: body, payload: payload)
        : _ForegroundPopupShowResult.unavailable;
    switch (result) {
      case _ForegroundPopupShowResult.shown:
        onForegroundDelivered?.call();
        return;
      case _ForegroundPopupShowResult.suppressed:
        onForegroundDelivered?.call();
        return;
      case _ForegroundPopupShowResult.unavailable:
  ...
  _ForegroundPopupShowResult _show({
    required int id,
    required String title,
    required String body,
    required String? payload,
  }) {
    if (!isForegroundGetter()) return _ForegroundPopupShowResult.unavailable;
    final context = contextGetter();
    if (context == null || !context.mounted) {
      return _ForegroundPopupShowResult.unavailable;
    }
    ...
    if (!_reserveVisibleDialog(id: id, signature: signature)) {
      return _ForegroundPopupShowResult.suppressed;
    }
    try {
      showDialog<void>(
        context: context,
        ...
        _releaseVisibleDialogReservation(id);
      });
      return _ForegroundPopupShowResult.shown;
    } catch (e, st) {
      _releaseVisibleDialogReservation(
        id,
        signature: signature,
        removeRecentSignature: true,
      );
      debugPrint('[ForegroundReminderPopupSink] show dialog failed: $e\n$st');
      return _ForegroundPopupShowResult.unavailable;
    }
  }

  void _releaseVisibleDialogReservation(
    int id, {
    String? signature,
    bool removeRecentSignature = false,
  }) {
    _visibleNavigators.remove(id);
    final reservedSignature = _visibleDialogSignatures.remove(id);
    _visibleDialogIds.remove(id);
    if (removeRecentSignature) {
      final recentSignature = signature ?? reservedSignature;
      if (recentSignature != null) {
        _recentVisibleDialogSignatures.remove(recentSignature);
      }
    }
  }
