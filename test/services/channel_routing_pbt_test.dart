...
void main() {
  ...
  group('P14 - 通道路由（Todo）', () {
    test(
      'push Todo 全部命中 NotificationSink.scheduleOnce；'
      'alarm Todo 全部命中 AlarmSink.scheduleFullScreen',
      () async {
        ...
        for (int iter = 0; iter < kIterations; iter++) {
          ...
          final pushTodoIds = <String>[];
          final alarmTodoIds = <String>[];
          ...
          for (int i = 0; i < n; i++) {
            ...
            todos.add(t);
            if (isPush) {
              pushTodoIds.add(t.id);
            } else {
              alarmTodoIds.add(t.id);
            }
          }
          ...
          // 且 Alarm 端没有对应 id。
          final pushIntIds = pushTodoIds.map(_todoIntId).toSet();
          final alarmIntIds = alarmTodoIds.map(_todoIntId).toSet();

          final notifScheduledIds =
              notif.scheduleOnceCalls.map((c) => c.id).toSet();
          final alarmScheduledIds =
              alarm.scheduleFullScreenCalls.map((c) => c.id).toSet();
          ...
          // Scheduler 的 sync 逻辑首轮不会触发 cancel（scheduledTodoKinds
          // 在构造后为空），这里一并断言，避免回归。
          expect(notif.cancelTodoReminderCalls, isEmpty);
          expect(alarm.cancelCalls, isEmpty);
        }
      },
    );

    test(
      'push ↔ alarm 切换时，旧通道的残留会被清理（双通道 cancel 语义）',
      () async {
        ...
        expect(
          alarm.cancelCalls,
          contains(_todoIntId(t.id)),
          reason: '切换通道时 alarm 侧也会被清（双通道 cancel 语义）',
        );
        expect(alarm.scheduleFullScreenCalls.length, 1);
        expect(alarm.scheduleFullScreenCalls.single.id, _todoIntId(t.id));
      },
    );
  });
  ...
  group('P14 - 通道路由（Goal）', () {
    test(
      'push Goal / alarm Goal 分发到对应 sink，且互不串扰',
      () async {
        final rng = Random(kSeed);
        for (int iter = 0; iter < kIterations; iter++) {
          final notif = _RecordingNotificationSink();
          ...
          final n = 1 + rng.nextInt(6);
          final pushGoalIds = <String>[];
          final alarmGoalIds = <String>[];
          ...
          for (int i = 0; i < n; i++) {
            ...
            goals.add(g);
            if (isPush) {
              pushGoalIds.add(g.id);
            } else {
              alarmGoalIds.add(g.id);
            }
          }

          await scheduler.syncGoals(goals);

          final pushIntIds = pushGoalIds.map(_goalIntId).toSet();
          final alarmIntIds = alarmGoalIds.map(_goalIntId).toSet();
  ...
  group('P14 - Habit 路径始终只走 push（通道单向约束）', () {
    test(
      'syncHabits 不会触发 AlarmSink（当前设计：habit 仅 push）',
      () async {
...
int _goalIntId(String goalId) => _idFor('goal_$goalId');
...
class _RecordingNotificationSink implements ReminderNotificationSink {
  ...
  Future<void> scheduleDaily({
    ...
    scheduleDailyCalls.add(_ScheduleDailyCall(
      id: id,
      hour: hour,
      minute: minute,
      weekdays: weekdays,
