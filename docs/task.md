...
## M9 课程表、纪念日深化（倒数日恢复可见）

依赖：M8

- [x] T9.1 课程表补学期、单双周、节次时间。
- [x] T9.2 课程进入 CalendarAggregator。
- [x] T9.3 倒数日恢复新增、搜索、日历聚合、导出和深链；旧数据继续兼容。
...
- [x] T15.6 发布 Release。
- [x] T15.7 应用内检查更新验证。
- [x] T15.8 404/API 路由闭环：后端 `/api/config` 暴露接口契约；静态契约测试覆盖 Flutter `/api` 调用和动态路径；后端回归测试覆盖高风险兼容路由。
...
- App 内检查更新可识别新版本。
- 核心回归清单通过。
- 404/API 路由契约测试和后端 no-404 回归测试通过。
...
2. M2 + M3 + M4 多次提醒完整闭环。
3. M5 + M6 + M7 时间足迹完整闭环。
4. M8 + M9 日历枢纽与课程/纪念日深化；倒数日保留可见入口和旧数据兼容。
5. M10 成就事件管线。
6. M11 + M12 共享空间。
7. M13 小组件增强。
8. M14 同步备份迁移。
